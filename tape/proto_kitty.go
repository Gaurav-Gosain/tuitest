...
func (p kittyKeys) baseKey(params csiParams, final byte) (string, bool) {
	...
	default:
		...
		// reaches the CSI form only with the explicit "1;mod" parameters.
		// Otherwise CSI R would be read as F3 when it is the cursor
		// position report. The leading parameter must be a bare 1: a
		// sub-parameter there describes an alternate key layout, which a
		// key identified by its final byte does not have, so its presence
		// means this is not a key report at all.
		if d, known := keyBySS3[final]; known && d.form == formSS3 &&
			len(params) >= 2 && len(params.group(0)) == 1 {
			return d.name, true
		}
		return "", false
	}
